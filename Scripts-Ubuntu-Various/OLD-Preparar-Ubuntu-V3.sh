...
if [[ $? -ne 0 ]] || [[ "$EUID" != 0 ]]; then
	...
	wget -O - http://download.invgate.net/assets/packages/debian/keyFile | sudo apt-key add -
	echo ""
	...
	wget http://download.invgate.net/assets/packages/debian/keyFile 
	sudo apt-key add keyFile
	echo "deb http://download.invgate.net/assets/packages/debian/binary /" > /etc/apt/sources.list.d/invgate-assets.list
	sudo apt-get update
	sudo apt-get -y install invgate-assets-client
EOF
	echo ""
...
comentario
	...
	# Set al nombre del equipo ARPPXXXX
